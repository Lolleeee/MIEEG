...
def wavelet_transform(signal: np.ndarray, wavelet: str = 'cmor1.5-1.0', bandwidth: List[float] = None, fs: float = None, num_samples: int = None) -> tuple[np.ndarray, np.ndarray]:
    """
    ...
    scales = pywt.frequency2scales(wavelet, frequencies, sampling_period=1/fs)
    ...
    if signal.ndim == 2:
        ...
        all_coeffs = np.zeros((signal.shape[0], len(signal), len(signal)), dtype=complex)
