...
def plot_reconstruction_distribution(original: Union[np.ndarray, torch.Tensor], reconstructed: Union[np.ndarray, torch.Tensor]) -> None:
    """
    ...
    plt.subplot(3, 1, 1)
    plt.hist(original.flatten(), bins=100, alpha=0.5, label='Original', color='blue', density=True)
    plt.hist(reconstructed.flatten(), bins=100, alpha=0.5, label='Reconstructed', color='orange', density=True)
    ...
    plt.subplot(3, 1, 3)
    plt.hist(np.abs(difference).flatten(), bins=100, alpha=0.7, color='green', density=True)
