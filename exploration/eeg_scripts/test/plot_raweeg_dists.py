...
def plot_random_channel_timeseries(eeg_data, n_channels=5):
    """
    Plot time series from N random EEG channels.
    ...
    # Select random channels
    random_channels = np.random.choice(n_total_channels, n_channels, replace=False)
    random_channels = range(0, n_channels) 
    # Create subplots
    ...
    for idx, ch in enumerate(random_channels):
        channel_data = eeg_data[ch, :]
        axes[idx].plot(channel_data, linewidth=0.5)
        axes[idx].set_title(f'Channel {ch} Time Series')
...
if __name__ == "__main__":
    data = np.load("/media/lolly/SSD/WAYEEGGAL_dataset/0.69subset_250_eeg_wav/patient1_trial1_seg0.npz")
    eeg = data['eeg']
    plot_random_channel_distributions(eeg, n_channels=6, bins=100)  
    plot_random_channel_timeseries(eeg, n_channels=6)